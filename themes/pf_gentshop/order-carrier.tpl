...
{else}
	<div id="carrier_area" class="opc-main-block p20">
...
<div class="order_carrier_content box">
	...
	{else}
		...
		{else}
									...
									<div>
										...
										{if !$option.unique_carrier}
											<table class="delivery_option_carrier{if isset($delivery_option[$id_address]) && $delivery_option[$id_address] == $key} selected{/if} resume table table-bordered{if $option.unique_carrier} hide{/if}">
												...
												{foreach from=$option.carrier_list item=carrier name=fkingcarrier}
												<tr>
													{if $smarty.foreach.fkingcarrier.first}
													...
													<td class="{if $carrier.product_list[0].carrier_list[0] eq 0} hide{/if}">
														{if $smarty.foreach.fkingcarrier.first}<input type="hidden" value="{$carrier.instance->id|intval}" name="id_carrier" />{/if}

														{if isset($carrier.instance->delay[$cookie->id_lang])}
															<i class="icon-info-sign"></i>
															{strip}
															{*ABU: veut afficher le delais de chaque transporteur, que le 1er (chelou ca d'ailleur)*}
															...
															{/if}
															{/strip}
															<ul class="carrier_productList">
															{foreach $carrier.product_list as $product}
																<li>
																	<i class="icon-fw icon-caret-right"></i>
																	<span>{$product.name|escape:'htmlall':'UTF-8'}</span>
																	{if isset($product.attributes) && $product.attributes}
																		&emsp;<i class="icon-align-left" title="<div class='text-left'>{$product.attributes|escape:'htmlall':'UTF-8'|replace:',':'<br/>'}</div>" data-toggle="tooltip" data-placement="right"></i>
																	{/if}
																</li>
															{/foreach}
															</ul>
			...
			{* ABU: recherche d'un produit avec l'attribut 'Métré' à true (id:186) *}
			{assign var='mustAgreeMetre' value=false}
			{foreach $carrier.product_list as $pdt}
				{if isset($pdt.array_attributes)}
					{foreach $pdt.array_attributes as $attr}
						{if $attr.id_attribute == 186}
							{assign var='mustAgreeMetre' value=$attr.public_group_name}
							{break}
						{/if}
					{/foreach}
				{/if}
			...
			{if $conditions AND $cms_id}
				...
				<p class="checkbox">
					...
					<label for="cgv">
						...
						<br />{l s='I also agree to receive the invoice by e-mail.'}
					</label>
				</p>
			{/if}
			{if $mustAgreeMetre}
				<p class="checkbox">
					<input type="checkbox" name="agreeMetre" id="agreeMetre" value="1" />
					<label for="agreeMetre">
						{l s='You chose to not accept the'}&nbsp;{$mustAgreeMetre|lower}.<br />
						{l s='I release So Nuit the responsibility of the surveying of my room.'}
					</label>
				</p>
			{/if}
			</div>
					...
					{if isset($virtual_cart) && $virtual_cart || (isset($delivery_option_list) && !empty($delivery_option_list))}
						<button type="submit" name="processCarrier" class="button btn btn-default standard-checkout button-medium">
							<span>
								...
								<small class="text-muted">({l s='obligation to pay'})</small>
...
{/if}
	{addJsDefL name=msg_agree_metre}{l s='You must agree to release any responsibility on So Nuit about the surveying of your room.' js=1}{/addJsDefL}
{/strip}