												...
												<tr>
													...
													<td class="{if $option.unique_carrier}first_item{/if}{if $first.product_list[0].carrier_list[0] eq 0} hide{/if}">
														...
														{if isset($first.instance->delay[$cookie->id_lang])}
															...
															{strip}
																<strong>{$first.instance->delay[$cookie->id_lang]|escape:'htmlall':'UTF-8'}</strong>
																<br />
															...
															{foreach $first.product_list as $product}
																...
																{strip}
																	...
																	{else}
																		{if isset($product.attributes) && $product.attributes}
																			<span title="<div class='text-left'>{$product.attributes|escape:'htmlall':'UTF-8'|replace:',':'<br/>'}</div>" data-toggle="tooltip" data-placement="right">{$product.name|escape:'htmlall':'UTF-8'}</span>
																		{else}
															...
															{foreach $option.carrier_list as $carrier}
																{if $carrier@iteration != 1}
																	{if $carrier.logo}
																		<img src="{$carrier.logo|escape:'htmlall':'UTF-8'}" alt="{$carrier.instance->name|escape:'htmlall':'UTF-8'}"/>
																	{else if !$option.unique_carrier}
																		<dd>{$carrier.instance->name|escape:'htmlall':'UTF-8'}</dd>
																	{/if}
																{/if}
															{/foreach}
														</dl>
