				...
				{if isset($ecotax_tax_breakdown)}
					{foreach $ecotax_tax_breakdown as $ecotax_tax_infos}
						...
						<tr style="line-height:6px;background-color:{cycle values='#FFF,#DDD'};">
							...
							{else} {*** ABU method ***}
								<td style="width: 20%; text-align: right;">20.000 %</td>
								{if !$use_one_after_another_method}
									<td style="width: 20%; text-align: right;">{if isset($is_order_slip) && $is_order_slip}- {/if}{displayPrice currency=$order->id_currency price=$ecotax_tax_infos.ecotax_tax_ht_abu}</td>
								{/if}
								<td style="width: 20%; text-align: right;">{if isset($is_order_slip) && $is_order_slip}- {/if}{displayPrice currency=$order->id_currency price=$ecotax_tax_infos.ecotax_tax_incl}</td>
							{/if}
						</tr>
						{/if}
					{/foreach}
