...
<div id="blogpage" class="blog-detail blog-container">
	<div class="inner">
	{if $is_active}
		<h1 class="blog-title p20">{$blog->meta_title}</h1>

		<div class="blog-meta p20">
		...
		<div class="blog-image p20">
			<img src="{$blog->preview_url}" title="{$blog->meta_title}"/>
		</div>
		{/if}

		{if $config->get('item_show_description',1)}
		<div class="blog-description p20">
			{$blog->description}
		</div>
		{/if}

		<div class="blog-content p20">{$blog->content}</div>

		{if trim($blog->video_code)}
		<div class="blog-video-code p20">
			<div class="inner">{$blog->video_code}</div>
		</div>
		{/if}
		...
		<div class="extra-blogs clearfix p20">
			{if !empty($samecats)}
			<div class="col-lg-6">
