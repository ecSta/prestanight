...
<div class="clearfix{if $PS_CATALOG_MODE} header_user_catalog{/if} inner">
{/if}
				...
				<div class="block_content">
					...
					<div class="cart_block_list{if isset($blockcart_top) && !$blockcart_top}{if isset($colapseExpandStatus) && $colapseExpandStatus eq 'expanded' || !$ajax_allowed || !isset($colapseExpandStatus)} expanded{else} collapsed unvisible{/if}{/if}">
										...
										<div class="cart-info">
											<div class="product-name">
												...
												<a class="cart_block_product_name" href="{$link->getProductLink($product, $product.link_rewrite, $product.category, null, null, $product.id_shop, $product.id_product_attribute)|escape:'html':'UTF-8'}" title="{$product.name|escape:'html':'UTF-8'}">
													{*$product.name|truncate:27:'..'|escape:'html':'UTF-8'*}
											...
											{foreach from=$customizedDatas.$productId.$productAttributeId[$product.id_address_delivery] key='id_customization' item='customization' name='customizations'}
												<li name="customization">
													<div data-id="deleteCustomizableProduct_{$id_customization|intval}_{$product.id_product|intval}_{$product.id_product_attribute|intval}_{$product.id_address_delivery|intval}" class="deleteCustomizableProduct">
														<a class="ajax_cart_block_remove_link" title="{l s='remove this product from my cart' mod='blockcart'}" href="{$link->getPageLink("cart", true, NULL, "delete=1&amp;id_product={$product.id_product|intval}&amp;ipa={$product.id_product_attribute|intval}&amp;id_customization={$id_customization}&amp;token={$static_token}", true)|escape:"html":"UTF-8"}" rel="nofollow">
															<i class="icon-trash-o"></i>
														</a>
													</div>
													{if count($customization.datas.$CUSTOMIZE_TEXTFIELD) > 1}
														{foreach from=$customization.datas.$CUSTOMIZE_TEXTFIELD item='customField' name='listCustomFields'}
															{$customField.value|replace:"<br />":" "|truncate:15:'..'|escape:'html':'UTF-8'},&nbsp;
														{/foreach}
													{elseif isset($customization.datas.$CUSTOMIZE_TEXTFIELD.0)}
						...
						<p class="cart-buttons text-center clearfix">
							<a id="button_order_cart" class="btn btn-default" href="{$link->getPageLink("$order_process", true)|escape:"html":"UTF-8"}" title="{l s='Check out' mod='blockcart'}" rel="nofollow">
								<span>
	...
	<div id="layer_cart">
		<div class="clearfix">
			...
			<div class="layer_cart_cart col-xs-12 col-md-6">
				...
				<div class="layer_cart_row">
					<strong class="dark">
						{l s='Shipping' mod='blockcart'}&nbsp;:{*if $display_tax_label}{if $priceDisplay == 1}{l s='(tax excl.)' mod='blockcart'}{else}{l s='(tax incl.)' mod='blockcart'}{/if}{/if*}
					</strong>
					<span class="ajax_cart_shipping_cost">
						{if $shipping_cost_float == 0}
							{l s='Frees!' mod='blockcart'}
						{else}
							{$shipping_cost}
						{/if}
					</span>
				...
				<div class="button-container">
					...
					<a class="btn btn-warning button pull-right check-out" href="{$link->getPageLink("$order_process", true)|escape:"html":"UTF-8"}" title="{l s='Proceed to checkout' mod='blockcart'}" rel="nofollow">
						<span>
...
{addJsDefL name=freeShippingTranslation}{l s='Freee!' mod='blockcart' js=1}{/addJsDefL}
