...
<ul class="display hidden-xs">
	<span class="pull-left">{l s='View:'}</span>
    <li id="grid"><a class="btn-tooltip" rel="nofollow" href="#" title="{l s='Grid'}"><i class="icon-th-large"></i><span>{{l s='Grid'}}</span></a></li>
...
<form id="productsSortForm{if isset($paginationId)}_{$paginationId}{/if}" action="{$request|escape:'html':'UTF-8'}" class="productsSortForm">
	<div class="select selector1">
		<span class="pull-left label-order" for="selectProductSort{if isset($paginationId)}_{$paginationId}{/if}">{l s='Sort by'}</span>
		<select id="selectProductSort{if isset($paginationId)}_{$paginationId}{/if}" class="selectProductSort form-control">
			{* ABU edit: on ne veut pas ce type trie par id *}
			{*<option value="{$orderbydefault|escape:'html':'UTF-8'}:{$orderwaydefault|escape:'html':'UTF-8'}" {if $orderby eq $orderbydefault}selected="selected"{/if}>{l s='Default'}</option>*}
			{if !$PS_CATALOG_MODE}
