...
function acceptCGV()
{
	...
	if (typeof msg_order_carrier != 'undefined' && $('#cgv').length && !$('input#cgv:checked').length)
	{
		...
		else alert(msg_order_carrier);

		return false;
	}
	else next = true;

	// ABU: accepte décharge en cas de non métré
	if (typeof msg_order_carrier != 'undefined' && $('#agreeMetre').length && !$('input#agreeMetre:checked').length)
	{
		next = false;
		if (!!$.prototype.fancybox)
		    $.fancybox.open([ {
	            ...
	            autoScale: true,
	            minHeight: 50,
	            content: '<p class="fancybox-error">' + msg_agree_metre + '</p>'
	        }], {
		        padding: 0
		    });
		else alert(msg_agree_metre);
	}
