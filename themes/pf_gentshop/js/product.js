...
$(document).ready(function()
{
	 ...
	 * ABU addon
	 */
	// Make product image follow scrolling
	$(window).on('scroll', function(e) {
		if( $('html, body').width() > 991 ) {
			var conBlock = $('.primary_block.row'),
				imgBlock = $('.primary_block.row .pb-left-column'),
				padValue = (parseInt($(window).scrollTop()) - 180) || 0,
				maxValue = conBlock.height() - imgBlock.height();

				imgBlock.stop(false, false).animate({
					paddingTop: ((imgBlock.height() + padValue) > conBlock.height() ? maxValue : padValue) +'px'
				}, 0);
		}
	});
	...
	$('#serviceInstallBtn').popover({
		...
		html: true,
		placement: $('html, body').width() > 991 ? 'left' : 'bottom',
	...
	if (typeof(contentOnly) != 'undefined' && !contentOnly)
	{
		if (!!$.prototype.fancybox)
			$('li:visible .fancybox, .fancybox.shown, #view_full_size .fancybox').fancybox({
...
$(document).on('click', '#color_to_pick_list li a', function(){
	var that = $(this);
	$(this).parents('fieldset:first').find('.chosenColorTitle').html( that.attr('title') );
});
...
function displayImage(domAAroundImgThumb, no_animation)
{
	...
	if (domAAroundImgThumb.attr('href'))
	{
		...
		var cur_src    = domAAroundImgThumb.attr('href');
		var new_src    = domAAroundImgThumb.attr('href').replace('.jpg', '-thickbox_default.jpg');
		var new_title  = domAAroundImgThumb.attr('title');
		var new_href   = domAAroundImgThumb.attr('href');

		if ($('#bigpic').attr('src') != new_src)
		{
			$('#view_full_size .span_link').attr({
				'href' : cur_src,
			});
			$('#bigpic').removeAttr('style').attr({
				'src' : new_src,
				'alt' : new_title,
				'title' : new_title
			}).load(function(){
			...
			// var bigpicNormalHeight = $('#bigpic').height();
			// $('#bigpic').css({
...
{
	serialScrollNbImagesDisplayed = parseInt($('#thumbs_list').width() / $('#thumbs_list_frame >li').outerWidth(true));
			...
			for (var z in tabValues)
				for (var a in attributesCombinations)
						...
						&& attributesCombinations[a]['attribute'] === decodeURIComponent(tabValues[z][1]))
					{
						...
						// ABU: select unecheck le group avant de recheck la bonne valeur
