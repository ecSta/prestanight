	...
	public function getProductByFilters($selected_filters = array())
	{
		...
		else
		{
			...
			else
			{
				...
				ORDER BY
				'. /* ABU : display favorites first */ (Tools::getProductsOrder('by', Tools::getValue('orderby'), true) == 'p.price' ? 'p.online_only desc,' : '' ) .'
				'. Tools::getProductsOrder('by', Tools::getValue('orderby'), true).' '.Tools::getProductsOrder('way', Tools::getValue('orderway')).' , cp.id_product'.
				' LIMIT '.(((int)$this->page - 1) * $n.','.$n), true, false);
			}
		}
