	...
	class AdminLeoblogBlogsController extends ModuleAdminControllerCore {
			...
			public function __construct()
			{
				...
				$this->fields_list = array(
					...
					'classification'  => array('title' => $this->l('Classification'), 'filter_key' => 'a!classification'),
					...
					// 'position'        => array('title' => $this->l('Position'), 'filter_key' => 'a!position', 'position' => 'position', 'class' => 'fixed-width-sm', 'align' => 'center'),
			...
			public function initPageHeaderToolbar()
			{
				...
				$blog      = new LeoBlogBlog( Tools::getValue('id_leoblog_blog'), $this->context->language->id );
			...
			public function renderForm()
			{
				...
				if (!$this->loadObject(true))
					...
					if( $this->object->image ){
						$thumb = __PS_BASE_URI__.'modules/leoblog/img/b/'. $this->object->image;
					}else {
						$thumb = '';
					}
					...
					$this->fields_form[0]['form'] = array(
						...
						'input' => array(
							...
							array(
								'type'     => 'text',
								'label'    => $this->l('Classification:'),
								'name'     => 'classification',
								'required' => true,
								'hint'     => $this->l('Fait ton cocon!').' A1-1-x'
							),
							array(
			                    'type'    => 'select',
				...
				if (Tools::isSubmit('viewblog') && ($id_leoblog_blog = (int)Tools::getValue('id_leoblog_blog')) && ($blog = new LeoBlogBlog($id_leoblog_blog, $this->context->language->id)) && Validate::isLoadedObject($blog))
					...
					if ( Tools::isSubmit('submitAddleoblog_blog') || Tools::isSubmit('submitAddleoblog_blogAndPreview') ) {
			            ...
			            else
			            {
			                ...
			                $this->copyFromPost($blog, 'blog');
			                 $folder = _LEOBLOG_BLOG_IMG_DIR_;

				            if ( isset($_FILES['image']) && isset($_FILES['image']['tmp_name']) && !empty($_FILES['image']['tmp_name']) ) {
								if(file_exists(_LEOBLOG_BLOG_IMG_DIR_.'b/'.$id_leoblog_blog))
									LeoBlogHelper::rrmdir(_LEOBLOG_BLOG_IMG_DIR_.'b/'.$id_leoblog_blog);
	 							 if (!$image = $this->_uploadImage($_FILES['image'],"", ""))
	                        	    return false;
			                    $blog->image = $image ;
				             }
			 ...
			 protected function _uploadImage($image, $image_w = '', $image_h = '')
		    {
		            ...
		            if (is_array($image) && (ImageManager::validateUpload($image, $this->max_image_size) === false) && ($tmp_name = tempnam(_PS_TMP_IMG_DIR_, 'PS')) && move_uploaded_file($image['tmp_name'], $tmp_name))
		            {
		                    ...
		                    $img_name = 'b-'.strtolower(str_replace('.'.$type,'',$image['name']).'.'.$type);
