...
<div class="panel-heading">
	...
	<span class="panel-heading-action">
        ...
        <div class="alert alert-info">{l s='Config of Group:'} {$group_title} - <a href="{$link->getAdminLink('AdminModules')}&configure=leosliderlayer&editgroup=1&id_group={$id_group}" alt={l s='Back to group'}>{l s='Back to group'}</a></div>
			...
			<li id="slides_{$slide.id_slide}">
				...
				<div style="float: right;margin-top: -5px;">
					{$slide.status}
                                            ...
                                            <ul class="dropdown-menu" style="border: none">
                                                <li style="background-color:#fff;border: none">
                                                   <a class="" href="{$link->getAdminLink('AdminModules')}&configure=leosliderlayer&editSlider=1&id_slide={$slide.id_slide}&id_group={$id_group}">
                                                       <i class="icon-edit"></i> {l s='Click to Edit'}
                                                   </a>
                                                </li>
                                                <li style="background-color:#fff;border: none">
                                                    <a class="color_danger btn-actionslider" data-confirm="{l s='Are you sure you want to delete this slider?'}" href="{$link->getAdminLink('AdminModules')}&configure=leosliderlayer&leoajax=1&action=deleteSlider&id_slide={$slide.id_slide}"><i class="icon-remove-sign"></i> {l s='Delete This slider'}</a>
                                                </li>
                                                <li style="background-color:#fff;border: none">
                                                   <a class="btn-actionslider" data-confirm="{l s='Are you sure you want to duplicate this slider?'}" href="{$link->getAdminLink('AdminModules')}&configure=leosliderlayer&leoajax=1&action=duplicateSlider&id_slide={$slide.id_slide}">
                                                       <i class="icon-film"></i> {l s='Duplicate This Slider'}
                                                   </a>
                                                </li>
                                            </ul>
                                        ...
                                        <div class="btn-group">
                                            <a class="btn btn-default {if $languages|count > 1}dropdown-toggle {else}slider-preview {/if}color_danger" href="{$previewLink}&id_group={$id_group}&id_slide={$slide.id_slide}"><i class="icon-eye-open"></i> {l s='Preview'}</a>
