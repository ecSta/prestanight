...
<fieldset>
    <div id="groupLayer" class="panel col-lg-12">
        <h3>{l s='Group List'}</h3>
        <div class="alert alert-info"><a href="http://www.leotheme.com/guides/prestashop16/leo-slider-layer/" target="_blank">{l s='Click to see configuration guide'}</a></div>
            ...
            <table class="table">
                <thead>
                    <tr class="nodrag nodrop">
                        ...
                        <th class="center fixed-width-xs"> <span class="title_box ">{l s='Status'}</span></th>

                        <th colspan="2">
                            <a href="{$link->getAdminLink('AdminModules')}&configure=leosliderlayer&addNewGroup=1" class="btn btn-default">
                                <i class="icon-plus"></i> {l s='Add new Group'}
                            </a>
                        </th>
                    </tr>
                </thead>
                <tbody>
                    {foreach from=$groups item=group}
                        <tr class=" odd">
                                ...
                                <div class="btn-group-action">
                                    <div class="btn-group pull-right">
                                        {if $group.id_leosliderlayer_groups != $curentGroup}
                                            <a href="{$link->getAdminLink('AdminModules')}&configure=leosliderlayer&editgroup=1&id_group={$group.id_leosliderlayer_groups}" title="{l s='Edit Group'}" class="edit btn btn-default">
                                                <i class="icon-pencil"></i> {l s='Edit'}
                                            </a>
                                        {else}
                                            <a href="#" title="{l s='Editting'}" class="btn " style="color:#BBBBBB">
                                                {l s='Editting'}
                                            </a>
                                        {/if}
                                        ...
                                        <ul class="dropdown-menu">
                                            <li>
                                                <a href="{$link->getAdminLink('AdminModules')}&configure=leosliderlayer&deletegroup=1&id_group={$group.id_leosliderlayer_groups}" onclick="if (confirm('{l s='Delete Selected Group?'}')) {
                                        return true;
                                    } else {
                                        event.stopPropagation();
                                        event.preventDefault();
                                    }
                                    ;" title="{l s='Delete'}" class="delete">
                                                ...
                                                {if $languages|count > 1}
                                                    {foreach from=$languages item=language}
                                                        ...
                                                        <a href="{$link->getModuleLink('leosliderlayer','preview', $arrayParam, null, $language.id_lang)}" class="group-preview">
                                                            <i class="icon-eye-open"></i> {l s='Preview For'} {$language.name}
                                                        </a>
                                                    {/foreach}
                                                {else}
                                                    <a class="" href="{$link->getAdminLink('AdminModules')}&configure=leosliderlayer&exportgroup=1&id_group={$group.id_leosliderlayer_groups}">
                                                        <i class="icon-eye-open"></i> {l s='Preview Group'}
                                                    </a>
                                                {/if}
                                                <a class="" target="_blank" href="{$exportLink}&id_group={$group.id_leosliderlayer_groups}">
                                                    <i class="icon-archive"></i> {l s='Export Group and sliders'}
                                                </a>
                                                {*<a class="" target="_blank" href="{$link->getAdminLink('AdminModules')}&configure=leosliderlayer&editgroup=1&correctGroup=1&id_group={$group.id_leosliderlayer_groups}">
                                                    <i class="icon-edit"></i> {l s='Correct Group Content'}
                                                </a>*}
                                                <a class="" href="{$link->getAdminLink('AdminModules')}&configure=leosliderlayer&editgroup=1&copylang=1&id_group={$group.id_leosliderlayer_groups}">
                                                    <i class="icon-edit"></i> {l s='Copy default language to other'}
                                                </a>
                                            </li>
                                        </ul>
                            ...
                            <td>
                                <a class="btn btn-default color_success" href="{$link->getAdminLink('AdminModules')}&configure=leosliderlayer&showsliders=1&id_group={$group.id_leosliderlayer_groups}"><i class="icon-film"></i> {l s='Manages Sliders'}</a>
                            </td>
                        </tr>
                    {/foreach}
            </table>
                            ...
                            <form method="post" enctype="multipart/form-data" action="{$link->getAdminLink('AdminModules')}&configure=leosliderlayer&importGroup=1">
                                <div class="row">
                                        <div class="form-group">
                                                ...
                                                <p class="help-block color_danger">{l s="Please upload *.txt only"}</p>
                                        </div>
                                        <div class="form-group">
                                            <label class="control-label col-lg-4" for="title_group">
                                                    {l s='Overide group or not:'}
                                            </label>
    ...
    $(document).ready(function() {
        ...
        $('#import_file_submit_btn').click(function(e){
            if($("#import_file-name").val().indexOf(".txt") != -1){
                if($("override_group_on").is(":checked")) return confirm("{l s='Are you sure to override group?'}");
                return true;
            }else{
                alert("{l s='Please upload txt file'}");
                $('#import_file').val("");
