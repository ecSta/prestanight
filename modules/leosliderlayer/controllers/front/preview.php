...
class LeoSliderLayerPreviewModuleFrontController extends ModuleFrontController {
    ...
    public function display() {
        ...
        if(Tools::getValue("preview") && isset($sliders)  && isset($sliders[0]["params"]) ){
            $slider_preview_data = trim( html_entity_decode((Tools::getValue("slider_preview_data"))));
            $slider_preview_data= json_decode( $slider_preview_data );
            if(  $slider_preview_data  ){
                foreach($slider_preview_data as $key=>$val){
                    $sliders[0][$key] = $val;
                }
            }
            $tmpSlider = array();
            $tmpSlider = $sliders[0]["params"];$sliders[0]["params"]=array();
            if(  $tmpSlider  ){
                foreach($tmpSlider as $key=>$val){
                    $sliders[0]["params"][$key] =  $val;
                }
            }
            $tmpSlider = $sliders[0]["video"];$sliders[0]["video"]=array();
            if(  $tmpSlider  ){
                foreach($tmpSlider as $key=>$val){
                    $sliders[0]["video"][$key] =  $val;
                }
            }
            $tmpSlider = $sliders[0]["layers"];$sliders[0]["layers"]=array();
            foreach($tmpSlider as $key=>$val){
                foreach($val as $k=>$v){
        ...
        foreach ($sliders as $key => $slider) {
            if(!Tools::getValue("preview")){
                ...
                $slider['video'] = Tools::jsonDecode(base64_decode($slider["video"]), true);
            }
            ...
            foreach($slider['layersparams'] as &$layerCss){
                    ...
                    if(!isset($layerCss['layer_link'])) $layerCss['layer_link'] = $slider['link'];
                    if(isset($layerCss['layer_caption'])) {
                        $layerCss['layer_caption'] = addslashes(str_replace("'", '&apos;', html_entity_decode(str_replace(array("\n", "\r", "\t"), '', utf8_decode($layerCss['layer_caption'])), ENT_QUOTES, 'UTF-8')));
                    }
            }
