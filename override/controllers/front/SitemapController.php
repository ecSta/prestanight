...
class SitemapController extends SitemapControllerCore
{
	...
	public function initContent()
	{
		...
		{
			$moduleblog = Module::getInstanceByName('blocksupplier');
			$helper     = LeoBlogHelper::getInstance();
			$blogs      = LeoBlogBlog::getListBlogs( null, $this->context->language->id , 1,  100, 'classification', 'ASC', array(), true );
			foreach ($blogs as $b => $blog) {
				$oBlog = new LeoBlogBlog( $blog['id_leoblog_blog'], $this->context->language->id );
				$blogs[$b]['helperLink'] = $helper->getBlogLink( get_object_vars($oBlog) );
			}
			$this->context->smarty->assign('listblogs', $blogs);
			$this->context->smarty->assign('display_blog_link', isset($moduleblog->active) ? (bool)$moduleblog->active : false);
		}
		else
			$this->context->smarty->assign('display_supplier_link', 0);
